...
// 使用案例
    ...
    wx.cloud.getTempFileURL({
      fileList: ['cloud://scnu-dev-iyplf.7363-scnu-dev-iyplf-1259750702/university/' + options.name + '.md'],
      success: res => {
    ...
    app.getText(app.docDir + 'university/' + options.name + '.md', (res) => {
      if (res.data) {
  ...
  jump_to_last: function (e) {
    for (var i = 0; i < this.data.menu.length; i++) {
      if (this.data.name + '' == this.data.menu[i]) {
        if (i == 0) {
          wx.redirectTo({
            url: 'index?name=' + this.data.menu[this.data.menu.length - 1],
          });
        } else {
          wx.redirectTo({
            url: 'index?name=' + this.data.menu[i - 1],
          });
        }
      }
    }
    ...
    for (var i = 0; i < this.data.menu.length; i++) {
      if (this.data.name + '' == this.data.menu[i]) {
        if (i == this.data.menu.length - 1) {
          wx.redirectTo({
            url: 'index?name=' + this.data.menu[0],
          });
        } else {
          wx.redirectTo({
            url: 'index?name=' + this.data.menu[i + 1],
  ...
  onShareAppMessage: function () {
    ...
    return {
      ...
      path: 'pages/university/book/index?name=' + this.data.name,
